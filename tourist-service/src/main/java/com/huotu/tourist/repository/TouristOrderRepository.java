...
public interface TouristOrderRepository extends JpaRepository<TouristOrder, Long>,
     ...
     * @return
     */
    @Query("select sum(t.orderMoney*t.touristGood.rebate/100) as om from TouristOrder as t " +
            "where t.touristGood.touristSupplier.id=?1 ")
     ...
     * @return
     */
    @Query("select count(t) from TouristOrder as t where t.touristBuyer=?1 and t.orderState in ?2")
     ...
     * @return
     */
    @Query("select count(t) from TouristOrder as t where t.touristBuyer=?1 and t.orderState=?2")
